...
import { pendingTrades } from './tradeMonitor';
...
const doTrading = async (clobClient: ClobClient, trades: UserActivityInterface[]) => {
    for (const trade of trades) {
        try {
            ...
            trade.bot = true;
            trade.botExcutedTime = RETRY_LIMIT;
        }
    }
};
...
const tradeExcutor = async (clobClient: ClobClient) => {
    ...
    while (true) {
        // Get unprocessed trades from pending queue
        const tradesToProcess = pendingTrades.filter(
            (trade) => !trade.bot && (!trade.botExcutedTime || trade.botExcutedTime < RETRY_LIMIT)
        );

        if (tradesToProcess.length > 0) {
            console.log(`💥 ${tradesToProcess.length} new transaction(s) found 💥`);
            spinner.stop();
            await doTrading(clobClient, tradesToProcess);
        } else {
